...
class Goodreceipt extends MY_Controller {
	...
	public function index()
	{
		...
		$sql = "SELECT
			...
			else if ($action == 'export_to_pdf')
			{
			    ...
			    $dompdf->render();
			    $dompdf->stream('performace-record-good-receipt.pdf');
			}
			...
			else
			{
				...
				$this->render('performance/index', array(
					...
					'period_start' => count($data_all) > 0 ? $data_all[count($data_all) - 1]->transaction_completed : '',
					'period_end' => count($data_all) > 0 ? $data_all[0]->transaction_completed : '',
				));
			}
	}
}
